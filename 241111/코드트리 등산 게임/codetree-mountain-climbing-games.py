...
for i in range(1, n + 1):
    ...
    # 등산 시뮬레이션 (점수 계산)
        ...
        m_index = operation[1]  # 케이블카 위치 (0-based)
        max_score = 0

        # 각 산에서 시작하여 시뮬레이션 수행
        for start in range(len(mountain)):
            current_score = 0
            pos = start
            ...
            while pos < len(mountain) - 1:
                ...
                if pos == m_index:
                    ...
                    # 케이블카를 타고 최저 높이 산으로 이동
                    min_pos = query_min(min_seg_tree, mountain, 1, 0, len(mountain) - 1, 0, len(mountain) - 1)
                    pos = min_pos
                    current_height = mountain[pos]
                    current_score += success_score
